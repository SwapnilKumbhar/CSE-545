...
NON_MALICIOUS_INTENTS = ["android.intent.action.MAIN", "android.intent.action.VIEW"]
...
def get_malicious_intents(intent_filter):
    ...
    for intent in intent_filter:
        if (
            intent.tag == "action"
            and ANDROID_NAME in intent.attrib.keys()
            and intent.attrib[ANDROID_NAME] not in NON_MALICIOUS_INTENTS
        ):
